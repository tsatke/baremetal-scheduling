    ...
    Self: Sized,
{
    ...
    fn prev_ptr(&self) -> &AtomicPtr<Self>;
}
...
impl<T: IntrusiveNode> LockFreeIntrusiveLinkedList<T> {
    ...
    pub fn append(&self, other: &Self) {
        'outer: loop {
            ...
            while !unsafe { (*other_tail).next_ptr() }.load(Relaxed).is_null() {
                other_tail = unsafe { (*other_tail).next_ptr() }.load(Relaxed);
            }
            ...
            loop {
                ...
                } else if unsafe { (*old_tail).next_ptr() }.compare_exchange(null_mut(), other_head, Release, Relaxed).is_ok()
                    && self.tail.compare_exchange(old_tail, other_tail, Release, Relaxed).is_ok() {
                    if unsafe { (*other_head).prev_ptr() }.compare_exchange(null_mut(), old_tail, Release, Relaxed).is_err() {
                        continue 'outer;
                    }
                    return;
                }
...
mod tests {
    ...
    fn test_append() {
        ...
        assert_eq!(node2, unsafe { (*node1).next_ptr().load(Relaxed) });
        assert_eq!(node5, unsafe { (*node6).prev_ptr().load(Relaxed) });
    }
    ...
    fn test_atomic_intrusive_linked_list() {
        ...
        assert_eq!(null_mut(), unsafe { (*node1).prev_ptr().load(Relaxed) });
        assert_eq!(null_mut(), unsafe { (*node1).next_ptr().load(Relaxed) });
        ...
        assert_eq!(node2, unsafe { (*node1).next_ptr().load(Relaxed) });
        assert_eq!(null_mut(), unsafe { (*node1).prev_ptr().load(Relaxed) });
        assert_eq!(null_mut(), unsafe { (*node2).next_ptr().load(Relaxed) });
        assert_eq!(node1, unsafe { (*node2).prev_ptr().load(Relaxed) });
        ...
        assert_eq!(node2, unsafe { (*node1).next_ptr().load(Relaxed) });
        assert_eq!(null_mut(), unsafe { (*node1).prev_ptr().load(Relaxed) });
        assert_eq!(node3, unsafe { (*node2).next_ptr().load(Relaxed) });
        assert_eq!(node1, unsafe { (*node2).prev_ptr().load(Relaxed) });
        assert_eq!(null_mut(), unsafe { (*node3).next_ptr().load(Relaxed) });
        assert_eq!(node2, unsafe { (*node3).prev_ptr().load(Relaxed) });
        ...
        assert_eq!(null_mut(), unsafe { (*node1).prev_ptr().load(Relaxed) });
        assert_eq!(null_mut(), unsafe { (*node1).next_ptr().load(Relaxed) });
        assert_eq!(null_mut(), unsafe { (*node2).prev_ptr().load(Relaxed) });
        assert_eq!(node3, unsafe { (*node2).next_ptr().load(Relaxed) });
        assert_eq!(node2, unsafe { (*node3).prev_ptr().load(Relaxed) });
        assert_eq!(null_mut(), unsafe { (*node3).next_ptr().load(Relaxed) });
        ...
        assert_eq!(null_mut(), unsafe { (*node2).prev_ptr().load(Relaxed) });
        assert_eq!(null_mut(), unsafe { (*node2).next_ptr().load(Relaxed) });
        assert_eq!(null_mut(), unsafe { (*node3).prev_ptr().load(Relaxed) });
        assert_eq!(null_mut(), unsafe { (*node3).next_ptr().load(Relaxed) });
